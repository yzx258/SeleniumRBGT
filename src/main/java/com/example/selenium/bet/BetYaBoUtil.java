...
public class BetYaBoUtil {
            ...
            for (int ch = 0; ch < chSize; ch++) {
                ...
                if (row_live != null && row_live.size() > 0) {
                    ...
                    if ((score.isEnabled() && score.isDisplayed()) && (node.isEnabled() && node.isDisplayed())) {
                        // 设定 - 节数
                        yaBoAccountInfo.setWhichSection(changeWhichSection(node.getText()));
    ...
    public static String changeWhichSection(String whichSection) throws Exception {
        if (whichSection.contains("第1节") || whichSection.contains("上半场")) {
            return "第1节";
        } else if (whichSection.contains("第2节")) {
            return "第2节";
        } else if (whichSection.contains("第3节") || whichSection.contains("下半场")) {
            return "第3节";
