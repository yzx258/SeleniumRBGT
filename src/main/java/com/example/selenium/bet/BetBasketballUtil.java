...
public class BetBasketballUtil {
    ...
    // 下注倍率:0.94倍率
    //private static String[] bl = new String[]{"2", "4", "8", "17", "35","73", "150", "292"};1417
    //private static String[] bl = new String[]{"10", "20", "42", "77", "159", "319", "666"};
    private static String[] bl = new String[]{"10", "20", "42", "79", "163", "328", "666"};
    // 是否需要
            ...
            if (SECOND.equals(djj)) {
                ...
                if (StrUtil.isNotBlank(zdtext1) && StrUtil.isNotBlank(kdtext1)) {
                    if ((Integer.parseInt(zdtext1.trim()) + Integer.parseInt(kdtext1.trim())) % 2 == betCacheSpec.getOddAndEven()) {
                        ...
                        d.sendMassage("该比赛已经红单，请关注该比赛,是否有出入[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][" + node + "][" + betCacheSpec.getMagnification() + "][" + betCacheSpec.getScore() + "]：[" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam() + "]");
                        ...
                        check = 2;
                        d.sendMassage("该比赛已经黑单，请关注该比赛,是否有出入[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][" + node + "][" + betCacheSpec.getMagnification() + "][" + betCacheSpec.getScore() + "]：[" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam() + "]");
                        // 设置四小时失效
                ...
                if (StrUtil.isNotBlank(zdtext2) && StrUtil.isNotBlank(kdtext2)) {
                    if ((Integer.parseInt(zdtext2.trim()) + Integer.parseInt(kdtext2.trim())) % 2 == betCacheSpec.getOddAndEven()) {
                        ...
                        if (betCacheSpec.getMagnification() == 7) {
                            ...
                            d.sendMassage("该比赛已经八黑，请关注该比赛,是否有出入[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][" + node + "][" + betCacheSpec.getMagnification() + "]：[" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam() + "]");
                            d.sendMassage("兄弟，很遗憾，江湖难免受阻，大势已去，一炮回到解放前，下次投注请慎重");
                            fifoCache.remove(betCacheSpec.getHomeTeam());
                        } else if(betCacheSpec.getMagnification() == 4){
                            addBuyRecord(betCacheSpec, 1);
                            d.sendMassage("该比赛已经五黑，请关注该比赛,是否有出入[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][" + node + "][" + betCacheSpec.getMagnification() + "]：[" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam() + "]");
                            fifoCache.remove(betCacheSpec.getHomeTeam());
                        } else {
                            check = 2;
                            betCacheSpec.setScore(zdtext2 + ":" + kdtext2);
                ...
                if (StrUtil.isNotBlank(zdtext3) && StrUtil.isNotBlank(kdtext3)) {
                    if ((Integer.parseInt(zdtext3.trim()) + Integer.parseInt(kdtext3.trim())) % 2 == betCacheSpec.getOddAndEven()) {
                        ...
                        if (betCacheSpec.getMagnification() == 2) {
                            d.sendMassage("该比赛已经三黑，请关注该比赛,是否有出入[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][" + node + "][" + betCacheSpec.getMagnification() + "][" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam() + "]");
    ...
    public static void sendBet(WebDriver driver, String zd, String kd, String djj, List<WebElement> trs, List<WebElement> td1,List<WebElement> td2) {
        ...
        if (betCacheSpec.getNumber() == 0) {
            ...
            if (flag) {
                ...
                driver.findElement(By.xpath("//*[@id=\"asianView\"]/div/div[3]/div[1]/div[2]/button")).click();
                SleepUtil.sleepUtil(3000);
                d.sendMassage("第一节比赛已购买,下注比赛[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][下注节点：" + betCacheSpec.getNode() + "][下注金额：" + bl[betCacheSpec.getMagnification()] + "]：" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam());
            ...
            if (flag) {
                ...
                driver.findElement(By.xpath("//*[@id=\"asianView\"]/div/div[3]/div[1]/div[2]/button")).click();
                SleepUtil.sleepUtil(3000);
                d.sendMassage("第二节比赛已购买,下注比赛[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][下注节点：" + betCacheSpec.getNode() + "][下注金额：" + bl[betCacheSpec.getMagnification()] + "]：" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam());
                d.sendMassage("[账户原金额：" + amount + "][下注金额：" + bl[betCacheSpec.getMagnification()] + "]" + "[账户剩余金额：" + getAmount(driver) + "]");
            }
            ...
            if (flag) {
                ...
                driver.findElement(By.xpath("//*[@id=\"asianView\"]/div/div[3]/div[1]/div[2]/button")).click();
                SleepUtil.sleepUtil(3000);
                d.sendMassage("第三节比赛已购买,下注比赛[单双选择："+(betCacheSpec.getOddAndEven() == 0 ? "单" : "双")+"][下注节点：" + betCacheSpec.getNode() + "][下注金额：" + bl[betCacheSpec.getMagnification()] + "]：" + betCacheSpec.getHomeTeam() + " VS " + betCacheSpec.getAwayTeam());
                d.sendMassage("[账户原金额：" + amount + "][下注金额：" + bl[betCacheSpec.getMagnification()] + "]" + "[账户剩余金额：" + getAmount(driver) + "]");
            }
        }
    }
    ...
    public static Boolean sendBetOk(WebDriver driver, List<WebElement> td1,List<WebElement> td2, BetCacheSpec betCacheSpec, String zd, String kd, String djj) {
        // 点击下注
        DingUtil d = new DingUtil();
        if(1 == betCacheSpec.getOddAndEven()){
            // 单购买记录
        ...
        }else if(0 == betCacheSpec.getOddAndEven()){
            // 双购买记录
